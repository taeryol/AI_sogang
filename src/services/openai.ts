...
3. **중요**: 답변에서 정보를 인용할 때 반드시 각주 형식으로 출처를 표시하세요
   - 형식: [1], [2], [3] 등의 숫자 각주
   - 예: "프로젝트는 2024년 1분기에 시작돼요[1]", "예산은 5억원입니다[2]"
   - 여러 출처: "이 정보는 확인되었어요[1][2]"
...
- 이모지 적절히 활용 (📌, 💡, ✅ 등)
- 핵심은 볼드체로 강조
- **각주는 반드시 [숫자] 형식만 사용** (예: [1], [2], [1][3])
- 각주 뒤에는 공백 없이 바로 이어서 작성`;

      const userPrompt = `**사용자 질문:**
${question}

**참고 문서:**
${contexts.map(ctx => `📄 [${ctx.source_number}] ${ctx.title} (청크 ${ctx.chunk_index})\n${ctx.content}`).join('\n\n---\n\n')}

위 문서들을 참고해서 사용자의 질문에 친절하고 자연스럽게 답변해주세요.
답변에서 정보를 인용할 때는 반드시 각주 형식으로 [1], [2] 등을 표시해주세요.`;
